...
دالة الترحيب الموحدة لتفعيلها عند الدخول أو عبر /start
...
دعم أمر /start وأي رسالة افتتاحية تلقائية عند الدخول لأول مرة
...
# معالجة تلقائية في حال ضغط المستخدم زر البدء الافتراضي في تيليجرام
...
markup.add(types.InlineKeyboardButton(place, callback_data=f"place_{place_type}{place}"))
markup.add(types.InlineKeyboardButton("✏️ مكان آخر (كتابة يدوية)", callback_data=f"place{place_type}_custom"))
